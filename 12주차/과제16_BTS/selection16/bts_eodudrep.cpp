...
void flow(bool st, int tmp, int d){
    if(bts[d].Ll == "" && bts[d].Rl == ""){
        for(auto &i : bts){
            if(i.Ll == bts[d].str) i.Ll = "";
            else if(i.Rl == bts[d].str) i.Rl = "";
        }
        bts.erase(bts.begin()+d);
        return;
    }
    else{
        if(st){
            if(bts[tmp].Rl == ""){
                for(auto &i : bts){
                    if(i.Ll == bts[d].str) i.Ll = bts[tmp].str;
                    else if(i.Rl == bts[d].str) i.Rl = bts[tmp].str;
                    else if(i.Ll == bts[tmp].str) i.Ll = bts[d].str;
                    else if(i.Rl == bts[tmp].str) i.Rl = bts[d].str;
                }
                string temp_name  = bts[tmp].str;
                bts[tmp].str = bts[d].str;
                bts[d].str = temp_name;
                flow(st, fdata(bts[tmp].Ll), tmp);
            }
            else flow(st, fdata(bts[tmp].Rl), d);
        }
        else{
            if(bts[tmp].Ll == ""){
                for(auto &i : bts){
            ...
            else flow(st, fdata(bts[tmp].Ll), d);
        }
    }
    return;
}
...
int main(){
    ...
    while(str != "quit"){
        ...
        else if(str == "depth") Depth(); 
        else if(str == "leaf") Leaf();
        cin >> str;
    }
}